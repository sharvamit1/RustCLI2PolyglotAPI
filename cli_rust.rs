...
impl CourseRepository {
    pub fn new(initial_data: Vec<Course>) -> Self {
        Self {
            courses: Arc::new(RwLock::new(initial_data)),
        }
    }

    pub async fn fetch_courses(&self, filtered: FilterCourses) -> Vec<Course> {
        let read_guard = self.courses.read().await;
        let mut results: Vec<Course> = read_guard
            .iter()
            .filter(|course| {
    ...
    pub async fn delete_course(&self, id: i8) -> Result<(), String> {
        let mut write_guard = self.courses.write().await;
        let len_before = write_guard.len();
        write_guard.retain(|c| c.id != id);
        
        if write_guard.len() == len_before {
            return Err(format!("Not Found Error: Course with ID {} does not exist!", id));
        }
        Ok(())
    }
}
